...
import useInformationSharingAgreementAudits from "@/use/use-information-sharing-agreement-audits"
import { formatDateTime } from "@/utils/formatters"
import { InformationSharingAgreementAudit } from "@/api/information-sharing-agreement-audits-api"

const props = defineProps<{ informationSharingAgreementId: number }>()
const informationSharingAgreementId = computed(() => props.informationSharingAgreementId)
...
const { items, totalCount, isLoading, fetch } = useInformationSharingAgreementAudits(
  informationSharingAgreementId.value,
  query
)

function makeSubtitle(item: InformationSharingAgreementAudit): string {
  if (item.user) {
