...
import db, { ArchiveItem, ArchiveItemCategory, ArchiveItemFile, Source } from "@/models"
...
export class CreateIntegrationService extends BaseService {
      ...
      const archiveItem = await ArchiveItem.create(
        {
          ...
          status,
          securityLevel,
          sourceId: this.attributes.source.id,
        },
        { transaction }
      )
          ...
          await ArchiveItemCategory.create(
            {
              ...
              setBySourceId: this.attributes.source.id,
            },
            { transaction }
          )
        }
