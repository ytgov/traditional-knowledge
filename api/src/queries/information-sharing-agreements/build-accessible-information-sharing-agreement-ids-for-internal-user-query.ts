 ...
 * accessible to an internal user.
 *
 * Requires replacements: { userId: number }
 *
 * NOTE: user must be internal!
 */
export function buildAccessibleInformationSharingAgreementIdsForInternalUserQuery() {
  return sql`
    (
      SELECT DISTINCT
          ...
          OR (
            ...
            AND EXISTS (
              ...
              WHERE
                ...
                AND user_groups.group_id = information_sharing_agreements.internal_group_id
            )
          )
        )
    )
...
export default buildAccessibleInformationSharingAgreementIdsForInternalUserQuery
